    ...
    <>
      <li className="fr-nav__item">
        <button
          ...
          {...(location.pathname.startsWith(`/${roleActivated}/demandes/coordinateurs`) || location.pathname.startsWith(`/${roleActivated}/historique/demandes/coordinateurs`) ? { 'aria-current': 'page' } : {})}
          onClick={onClickMenu}>
        ...
        >
          <ul className="fr-menu__list" style={{ width: '24rem' }}>
            ...
            <li>
              <Link className="fr-nav__link" to={`/${roleActivated}/historique/demandes/coordinateurs`}
                {...(location.pathname.startsWith(`/${roleActivated}/historique/demandes/coordinateurs`) ? { 'aria-current': 'page' } : {})}
                onClick={() => trackEvent({ category: 'historique-demande-coordinateurs', action: `click-${roleActivated}` })}
              >
                Historique des demandes de coordinateurs trait&eacute;es
              </Link>
            </li>
          </ul>
        </div>
